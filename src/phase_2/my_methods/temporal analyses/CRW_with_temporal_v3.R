...
CRW_FUN_PAR <- function(temp_FP, land_mask, num_tracks, lenndf, x, IDs){
  ...
  x <- foreach(n=1:num_tracks, .combine = 'rbind') %do%{
    print(n)
    ...
    draws <- lenn
    lenn_new_sim_df = 0
    while (lenn_new_sim_df<(as.numeric(lenn))){
      new_dis <- rlnorm(as.numeric(draws),meanlog = 11.5943286, sdlog=0.9036841) # values pulled from fitdistr for relevant time period
      new_ang <- runif(as.numeric(draws),min=0,max=359) # uniform
      new_x <- bloop$Longitude[1]
      new_y <- bloop$Latitude[1]
      timee <- ymd_hms(bloop$date[1])
      for(i in 2:(as.numeric(draws))){
        new_x[i] <- new_x[i-1] + new_dis[i]*cos(new_ang[i])
        new_y[i] <- new_x[i-1] + new_dis[i]*sin(new_ang[i])
        timee[i] <- ymd_hms(timee[i-1] + i)
      }
      
      newptsdf <- data.frame(X=new_x, Y=new_y)
      
      testpoints <- st_as_sf(x = newptsdf, coords = c("X", "Y")) %>% 
        st_set_crs(st_crs(land_mask))
      ...
      if (all(is.na(in_land$type))){
        e <- data.frame("Animal_ID" = IDs[n], "Longitude" = new_x, "Latitude" = new_y, 
                        "date" = timee, "new_dis" = new_dis, "new_ang" = new_ang, num = lenn_new_sim_df:(as.numeric(lenn)-1))
        bloop <- rbind(bloop, e)
        lenn_new_sim_df <- nrow(bloop)
      }else{
        print("FAIL")
        # find the index just prior to the first fail point
        stop <- min(c(which((is.na(in_land$type))==FALSE), which((-1708182<new_x)==F), which((new_x<9844089)==F), which((1185132<new_y)==F), which((new_y<9310913)==F)))
        cat("STOP IS",stop)
        e <- data.frame("Animal_ID" = IDs[n], "Longitude" = new_x[1:stop], "Latitude" = new_y[1:stop], 
                        "date" = timee[1:stop], "new_dis" = new_dis[1:stop], "new_ang" = new_ang[1:stop], num = 1:stop)
        bloop <- rbind(bloop,e)
        lenn_new_sim_df <- nrow(bloop)
        draws = as.numeric(lenn) - lenn_new_sim_df
        cat("NUM IS",lenn_new_sim_df)
      } # end of elseif
    } # end of while
    bloop$iter <- n # this is effectively assigning the sim number of 1:num_tracks
  } # end of num_tracks foreach
} # end of function
...
for(sc in unique(sppcol)){
  ...
  sim_CRW_pts_L <- lapply(X=1:1, FUN = CRW_FUN_PAR,temp_FP = temp_FP, land_mask = land_mask, num_tracks = num_tracks, lenndf = lenndf, IDs = unique(temp_FP$Animal_ID))
  tosave <- do.call(rbind, sim_CRW_pts_L)
  # SAVE THIS OUT
  # e.g. savedL[[sc]] <- tosave
