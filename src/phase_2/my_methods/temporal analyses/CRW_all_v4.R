...
for(sc in unique(sppcol)){
  ...
  sim_CRW_pts_L <- parallel::mclapply(X=c(1:1000), FUN = CRW_FUN_PAR, temp_FP = temp_FP, land_mask = land_mask, num_tracks = num_tracks, lenndf = lenndf, IDs = unique(temp_FP$Animal_ID))
  save_spp_col_list <- append(save_spp_col_list,sim_CRW_pts_L)# SAVE THIS OUT
} # end of species / colony loop

save(save_spp_col_list,file="CRW_v4_all.Rdata")
end <- Sys.time()
