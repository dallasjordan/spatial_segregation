# Processing CRW values to get p-values of 'CRW method' 
# Reads in R-data files of CRW generated dataframes in a list. Reads in aggregated, spring, summer, 
# and fall. Takes these lists, makes a giant dataframe, adds iteration numbers, then calculates p-values. 
# Dallas Jordan May 1 2022
# Changes May 5 2022: You realized that you were calculating spring as March April May, summer as June July Aug, etc. 
# You are excluding March April May since there wasn't enough data...so you have to recalculate "spring" as June-July,
# summer as Aug. Sept., etc. This requires re-running everything but "all". 
# Setup -------------------------------------------------------------------
...
# load in Rdata files from CRW_temporal_vX.R script

setwd("/Users/dallasjordan/Desktop/StonyBrook/SoMAS/Thesis/R/spatial_segregation/")

load(file="./data/overlap_sensitivity/CRW_v4_all.Rdata")
spp_col_list_all <- save_spp_col_list
load(file="./data/overlap_sensitivity/CRW_v4_junejuly.Rdata")
spp_col_list_spring <- save_spp_col_list
load(file="./data/overlap_sensitivity/CRW_v4_augustseptember.Rdata")
spp_col_list_summer <- save_spp_col_list
load(file="./data/overlap_sensitivity/CRW_v4_octobernovember.Rdata")
spp_col_list_fall <- save_spp_col_list

load(file="./data/overlap_sensitivity/CRW_v4_test.Rdata")
spp_col_list_all <- save_spp_col_list
...
all_df <- list_to_df_add_iteration_number(spp_col_list_all)
spring_df <- list_to_df_add_iteration_number(spp_col_list_spring)
summer_df <- list_to_df_add_iteration_number(spp_col_list_summer)
fall_df <- list_to_df_add_iteration_number(spp_col_list_fall)

test_df <- list_to_df_add_iteration_number(spp_col_list_all)

# Calculate UDs and p-values ----------------------------------------------

# section break here to help stay organized!

# Prep data, set up parallelization, and select season ---------------------------------------------
# get variables set up and then assign a time period to sim_CRW_pts to process. Run the code, 
# save the output, then reassign sim_CRW_pts. 
...
# IMPORTANT! CHANGE HERE TO GET DIFFERENT AGGREGATE/SEASONS:
sim_CRW_pts <- fall_df
sim_CRW_pts <- sim_CRW_pts %>% group_by(Animal_ID) %>% mutate(n=n()) %>% filter(n>5000) # has to be greater than 5 for 1000 iterations, so 5000
sim_CRW_pts$Animal_ID <- paste( "sim_", sim_CRW_pts$Animal_ID, sep="")
sim_CRW_pts <- split(sim_CRW_pts,sim_CRW_pts$iteration_number)

# KDE and UD code starts here -------
UD_pvalues <- function(sim_CRW_pts){
  ...
  proj4string(all_data_1) <- CRS(pdc_mercator_proj) # I tested LCEA and PDC mercator - they had the exact same results here
  ...
  # Alter lines 154-157 with the results of "names" to see individual tracks. You might not want to do this because you have thousands of tracks
  names <- names(results)
  ...
  c_comb <- c(alab_UDOI_95_test_stat, alab_UDOI_50_test_stat, mltl_UDOI_95_test_stat, mltl_UDOI_50_test_stat, mbtb_UDOI_95_test_stat, mbtb_UDOI_50_test_stat, mlmb_UDOI_95_test_stat, mlmb_UDOI_50_test_stat, tltb_UDOI_95_test_stat, tltb_UDOI_50_test_stat)
  ...
  # 
  # rand_out_pts_birds[[j]] <- sim_CRW_sp this is just gonna be too big
  # rand_out_plot_all[[j]] <- p_all don't need these commented out ones, can re run later if I do
  ...
  print("DONE GETTING TEST STATS")

  rand_out_table_birds <- data.frame(rand_out_table_birds) 
  colnames(rand_out_table_birds) <- c("ALAB95", "ALAB50", "MLTL95", "MLTL50", "MBTB95", "MBTB50", "MLMB95", "MLMB50", "TLTB95", "TLTB50")
  return(rand_out_table_birds)
}

sim_overlap_values <- parallel::mclapply(sim_CRW_pts, FUN = UD_pvalues)
save(sim_overlap_values, file="./data/overlap_sensitivity/sim_overlap_values_octobernovember.RDS")
 ...
 # load object, assign iteration numbers, save as csv
load("/Users/dallasjordan/Desktop/StonyBrook/SoMAS/Thesis/R/spatial_segregation/data/overlap_sensitivity/sim_overlap_values_spring.RDS")
