# Permutation test with permutation of track labels - ternLAAL and ternBFAL
...
tk <- rbind(lt,bt)
tk <- tk[,c(1,3,4)]
sp::coordinates(tk) <- c("x", "y")
proj4string(tk) <- CRS("+proj=longlat +datum=WGS84 +no_defs") # placeholder
tk <- spTransform(tk,lcea)
grid_input <- calculate_sp_obj_extent(tk,0.1)

a_ud <-  kernelUD(tk, grid=grid_input,same4all=T,extent=0.1,h=150000)
image(a_ud)
...
resample_this <- rbind(add_lt,add_bt)
...
for (i in 1:iter){
  ...
  # average mid_iter #
  mid_iter_holder <- numeric(length = 572)
  ...
  # average tern_iter #
  tern_iter_holder <- numeric(length = 572)
