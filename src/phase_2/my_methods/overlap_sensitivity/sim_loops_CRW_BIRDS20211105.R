### CRW code for simulations for REAL BIRD TRACKS
...
setwd("/Users/dallasjordan/Desktop/StonyBrook/SoMAS/Thesis/R/spatial_segregation/tern_postbreeding_exports/")
d_spps_bfal_tern <- list.files(pattern = "BFAL.*\\.csv$", recursive=T)
d_spps_bfal_tern <- lapply(d_spps_bfal_tern, function(x) read.csv(x, header=T))
d_spps_laal_tern <- list.files(pattern = "LAAL.*\\.csv$", recursive=T)
...
for(i in 39:73){
  ...
  w$ID <- paste("BFAL_TERN_", i, sep="")
  w$GMT <- ymd_hms(w$GMT)
  ee <- w %>% dplyr::select(ID, GMT, zm.lon, zm.lat)
  names(ee) <- names(d_spps_bfal_ternID)
  d_spps_bfal_ternID <- rbind(d_spps_bfal_ternID, ee)
}
colnames(d_spps_bfal_ternID) <- c("ID", "GMT", "Longitude", "Latitude")
...
for(i in 46:89){
  ...
  w$ID <- paste("LAAL_TERN_", i, sep="")
  w$GMT <- ymd_hms(w$GMT)
  ee <- w %>% dplyr::select(ID, GMT, xm.lon, xm.lat)
  names(ee) <- names(d_spps_bfal_ternID)
  d_spps_laal_ternID <- rbind(d_spps_laal_ternID, ee)
}
...
newLL <- reproj(cbind(d_birds$Longitude, d_birds$Latitude), target = "+proj=cea +lat_0=0 +lat_ts=0 +lon_0=180 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs +type=crs", source = "+proj=longlat +datum=WGS84 +no_defs")
...
d_birds_traj <- as.ltraj(cbind(d_birds$Longitude2, d_birds$Latitude2), date=d_birds$GMT, id = d_birds$ID, typeII = T,
                        proj4string = CRS("+proj=cea +lat_0=0 +lat_ts=0 +lon_0=180 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs +type=crs"))
...
for(j in 1:100){
  ...
  for(i in unique(first_pts$Animal_ID[first_pts$SPP == "LAAL"])){
    ...
    for(z in 1:1){
      new_ang <- sample(angles_LAAL, lenn, replace=T)
      new_dis <- sample(dist_LAAL, lenn, replace=T)
      
      bloop <- temp_pts %>% dplyr::select(Animal_ID, Longitude, Latitude,date, new_dis, new_ang, num)
      for(k in 2:lenn){
        k <-2 
        new_x <- bloop$Longitude[k-1] + new_dis[k-1]*cos(new_ang[k-1])
        new_y <- bloop$Latitude[k-1] + new_dis[k-1]*sin(new_ang[k-1])
        timee <- ymd_hms(bloop$date[k-1]) + (k-1)
        e <- data.frame("Animal_ID" = i, "Longitude" = new_x, "Latitude" = new_y, "date" = timee, "new_dis" = new_dis[k-1], "new_ang" = new_ang[k-1], "num" = k-1)
        bloop <- rbind(bloop, e)}
  ...
  for(i in unique(first_pts$Animal_ID[first_pts$SPP == "BFAL"])){
    ...
    for(z in 1:1){
      new_ang <- sample(angles_BFAL, lenn, replace=T)
      new_dis <- sample(dist_BFAL, lenn, replace=T)
  ...
  proj4string(all_data_1) <- CRS(lcea)
