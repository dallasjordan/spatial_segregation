...
track.data <- read.table("ALL_TRACK_POINTS.txt")
...
SST_SLA_km4<-cbind(track.data[1,],0,0,0,0,0,0,0)
names(SST_SLA_km4)<-names(track.data)
n_ <- ncol(SST_SLA_km4)
...
for (yrs in 2010:2015)  {
  ...
  for (mths in 1:12) {
    ...
    for (dys in 1:numdays) {
      ...
      points <- cbind(track.data$POINT_X,track.data$POINT_Y)
      ...
      ## I can't remember why I had to do this-- probably not relevant fo ryou 
      yr<-strsplit(as.character(yrs),split=datesplit)[[1]][2]
      ...
      temp2 <- cbind(track.data, date2, yrs, mths, dys, SST, SLA, dfronts)
