...
for (yrs in 2008:2012){
  ...
  for (mths in 1:12) {
    ...
    n_ <- ncol(wind_data)
    names(wind_data)[n_]<-paste("wind_direction10M")
    names(wind_data)[n_ -1]<-paste("wind_speed10M")
    names(wind_data)[n_ -2]<-paste("daily_avg_v10m")
    names(wind_data)[n_ -3]<-paste("daily_avg_u10m")
    ...
    for (dys in 1:numdays) {
      ...
      if (nrow(points)>0){
        ...
        wind_u10<-as(wind_ncdf[1,,,], "Raster")
        wind_u.df= raster::as.data.frame(wind_u10, xy = TRUE)
        wind_U10M <- wind_u.df
        ...
        wind_u10v2<-as(wind_ncdf2[1,,,], "Raster")
        wind_u.df2= raster::as.data.frame(wind_u10v2, xy = TRUE)
        ...
        wind_extracted <- extract(u_stack10M, points,buffer=300000, fun=mean)
        ...
        temp2$daily_avg_u10m <- temp.means
        ...
        ## this reads a netcdf, converts to raster, converts to dataframe
        wind_ncdf <- read_ncdf(filename_left)
        wind_v10<-as(wind_ncdf[4,,,], "Raster")
        wind_v.df= raster::as.data.frame(wind_v10, xy = TRUE)
        wind_V10M <- wind_v.df
        ...
        wind_ncdf2 <- read_ncdf(filename_right)
        wind_v10v2<-as(wind_ncdf2[4,,,], "Raster")
        wind_v10.df2= raster::as.data.frame(wind_v10v2, xy = TRUE)
        wind_V10M2 <- wind_v10.df2
        ...
        # extract from this raster, which should just include points that are present in "points" (movement data)
        wind_extracted <- extract(v_stack10M, points,buffer=300000, fun=mean)
        ...
        temp2$daily_avg_v10m <- temp.means
        ...
        temp2$wind_speed10M <- holder_ff
        temp2$wind_direction10M <- holder_dd
    ...
    output.name2 <-  paste(island,species,"_wind10M_extracted",yrs,mths,".txt",sep="")
