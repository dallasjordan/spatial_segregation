...
both <- rbind(LAAL,BFAL)
    ...
    results <- kernelUD(both_1, grid=grid_input,same4all=T,extent=0.12)
    image(results)
    ...
    original_BA_50 <- kerneloverlaphr(results, method="BA", percent=50)
        ### MIDWAY ###
        BA_95 <- original_BA_95 # 	0.8145530	
        BA_50 <- original_BA_50 #   0.8145530	
        test_stat_95 <- 0.7171074 # manually putting this in because its easier then calling from matrix
        test_stat_50 <- 0.03897323	 # manually putting this in because its easier then calling from matrix
        ### TERN ###
        BA_95 <- original_BA_95 # 	0.5874273
        BA_50 <- original_BA_50 # 	0.5874273
        test_stat_95 <- 0.2816425 # manually putting this in because its easier then calling from matrix
        test_stat_50 <- 0	 # manually putting this in because its easier then calling from matrix
...
for (i in 1:iter){
  ...
  kernel.boot <- kernelUD(final_iter, grid=grid_input, same4all = T, extent=0.12)
...
### MIDWAY ###
p_value # p<0.001
mean_value # 0.2332471
sd_value # 0.03271706

### TERN ###
p_value # p<0.001
mean_value # 0.2335277
sd_value # 0.02828188
